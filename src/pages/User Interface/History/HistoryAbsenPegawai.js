...
export default function HistoryAbsenPegawai() {
    const { id } = useParams();
    ...
    const fetchHistoryAbsen = async () => {
        const url = user.dashboard_type === "pegawai"
            ? `/absensi/by-pegawai/${user.id}`
            : `/absensi/by-pegawai/${pegawai_id}`;
    ...
    const renderStatusBadge = (status) => {
        if (!status) {
            return (_jsx("span", { className: "inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-gray-100 text-gray-600", children: "-" }));
        }
        const base = "inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold capitalize";
        switch (status) {
            case "hadir":
                return _jsx("span", { className: `${base} bg-green-100 text-green-700`, children: "Hadir" });
            case "izin":
                return _jsx("span", { className: `${base} bg-yellow-100 text-yellow-700`, children: "Izin" });
            case "sakit":
                return _jsx("span", { className: `${base} bg-blue-100 text-blue-700`, children: "Sakit" });
            case "cuti":
                return _jsx("span", { className: `${base} bg-indigo-100 text-indigo-700`, children: "Cuti" });
            case "libur":
                return _jsx("span", { className: `${base} bg-gray-200 text-gray-700`, children: "Libur" });
            case "alpha":
                return _jsx("span", { className: `${base} bg-red-100 text-red-700`, children: "Alpha" });
            case "sudah_masuk":
    ...
    const renderProgressBadge = (status) => {
        ...
        switch (status) {
            case "belum_masuk":
                return (_jsx("span", { className: `${base} bg-gray-100 text-gray-600`, children: "Belum Masuk" }));
            case "sudah_masuk":
                return (_jsx("span", { className: `${base} bg-purple-100 text-purple-700`, children: "Sudah Masuk" }));
            case "sudah_pulang":
                return (_jsx("span", { className: `${base} bg-green-100 text-green-700`, children: "Sudah Pulang" }));
