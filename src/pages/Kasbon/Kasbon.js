...
export default function Kasbon() {
    ...
    const handleApproveKasbon = async (row) => {
    ...
    const columns = [
        ...
        },
        {
            ...
            cell: (row) => {
                ...
                return (_jsx("span", { className: `px-3 py-1 rounded-full text-white font-medium ${color}`, children: formattedStatus }));
            },
            width: "120px",
        },
        {
            header: "Actions",
            width: "160px",
            cell: (row) => (_jsxs("div", { className: "flex gap-2", children: [row.status === "pending" && (_jsx("button", { onClick: () => handleApproveKasbon(row), className: "bg-yellow-500 hover:bg-yellow-600 text-white p-2 rounded flex items-center justify-center", title: "Approve / Reject", children: _jsx(CheckCircle, { size: 18 }) })), _jsx("button", { onClick: () => handleEdit(row), className: "bg-blue-600 hover:bg-blue-700 text-white p-2 rounded flex items-center justify-center", title: "Edit", children: _jsx(Edit, { size: 16 }) }), _jsx("button", { onClick: () => handleDelete(row.id), className: "bg-red-600 hover:bg-red-700 text-white p-2 rounded flex items-center justify-center", title: "Hapus", children: _jsx(Trash, { size: 16 }) })] })),
        },
    ];
    // SEARCH + FILTER
    const filteredData = kasbonList
        .filter((k) => k.pegawai?.name?.toLowerCase().includes(localSearch.toLowerCase()))
        .filter((k) => selectedCompany === "all" ? true : k.company_id === selectedCompany);
    return (_jsxs(_Fragment, { children: [_jsx(PageMeta, { title: "Kasbon Pegawai", description: "Daftar Kasbon Pegawai" }), _jsx(PageHeader, { pageTitle: "Kasbon Pegawai", titleClass: "text-[32px] dark:text-white", rightContent: _jsx("button", { onClick: () => navigate("/add-kasbon"), className: "bg-blue-600 hover:bg-blue-700 text-white font-medium px-5 py-2 rounded-xl", children: "+ Tambah" }) }), _jsx("div", { className: "space-y-5 sm:space-y-6", children: _jsxs(ComponentCard, { className: "dark:bg-gray-800 dark:border-gray-700", children: [_jsx("div", { className: "w-full bg-gray-100 dark:bg-gray-900 p-4 rounded-xl border border-gray-300 dark:border-gray-700 mb-4", children: _jsxs("div", { className: "flex flex-wrap gap-6 items-end", children: [_jsxs("div", { className: "flex flex-col w-72", children: [_jsx("label", { className: "text-sm font-medium mb-1 text-gray-900 dark:text-white", children: "Cari Pegawai" }), _jsx("input", { type: "text", placeholder: "Cari pegawai...", value: localSearch, onChange: (e) => setLocalSearch(e.target.value), className: "border px-3 py-2 rounded-lg w-72 bg-white dark:bg-gray-700 dark:text-white" })] }), dashboardType === "superadmin" && (_jsxs("div", { className: "flex flex-col", children: [_jsx("label", { className: "text-sm font-medium mb-1 text-gray-900 dark:text-white", children: "Company" }), _jsxs("select", { value: selectedCompany, onChange: (e) => setSelectedCompany(e.target.value === "all"
                                                    ? "all"
