...
export default function CutiPage() {
    ...
    const storageUrl = `${API_URL}/storage/`;
    ...
    const handleApprove = async (row) => {
        ...
        try {
            const token = localStorage.getItem("token");
            await api.put(`/cuti/${row.id}/approve`, {
                catatan: note || "",
    ...
    const columns = [
        ...
        },
        {
            header: "Nama Pegawai",
            accessor: "pegawai_id",
            cell: (row) => (_jsx("div", { children: row.pegawai?.name || "-" })),
        },
        {
            header: "Lokasi Pegawai",
            accessor: "pegawai_id",
            cell: (row) => (_jsx("div", { children: row.pegawai?.lokasi?.nama_lokasi || "-" })),
        },
        {
            header: "Jenis Cuti",
            accessor: "jenis_cuti",
        ...
        },
        {
            ...
            cell: (row) => {
                ...
                return (_jsx("span", { className: `px-2 py-1 rounded text-sm font-medium ${st.bg} ${st.text}`, children: st.label }));
            },
        },
        {
            header: "Foto",
            accessor: "foto",
            cell: (row) => {
                return row.foto ? (_jsx("img", { src: storageUrl + row.foto, alt: "Foto Cuti", className: "w-20 aspect-square object-cover rounded-full cursor-pointer", onClick: () => handlePreview(row.foto) })) : ("-");
            },
        },
        {
        ...
        },
        {
            ...
            cell: (row) => {
                ...
                return (_jsxs("div", { className: "flex gap-2", children: [isPending && (_jsxs(_Fragment, { children: [_jsx("button", { onClick: () => handleApprove(row), className: "bg-green-600 hover:bg-green-700 text-white p-2 rounded", title: "Approve", children: _jsx(Check, { size: 18 }) }), _jsx("button", { onClick: () => handleReject(row), className: "bg-red-600 hover:bg-red-700 text-white p-2 rounded", title: "Reject", children: _jsx(X, { size: 18 }) })] })), _jsx("button", { onClick: () => handleDelete(row.id), className: "bg-red-600 hover:bg-red-700 text-white p-2 rounded", children: _jsx(Trash2, { size: 18 }) })] }));
            },
        },
    ];
    // FILTERING
    const filteredCutis = cutis
        .filter((c) => (c.pegawai?.name || "")
        .toLowerCase()
        .includes(localSearch.toLowerCase()))
        .filter((c) => selectedCompany === "all" ? true : c.company_id === selectedCompany);
    const handlePreview = (foto) => {
        if (!foto)
            return;
